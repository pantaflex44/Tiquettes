...
function ScrollBar(
	...
	downArrow.setAttributeNS(
		null,
		"y",
		scrollBarY + scrollBarHeight - arrowWidth,
	);
	...
	this.scrollToStep = function (newStep) {
		...
		var cursorPos =
			newStep *
			((this.scrollBarHeight - 2 * this.arrowWidth - this.cursorHeight) /
				this.steps);
		this.cursor.setAttributeNS(
			null,
			"transform",
			"translate(0," + cursorPos + ")",
		);
		...
		this.content.setAttributeNS(
			null,
			"transform",
			"translate(0," + contentPos + ")",
		);
	...
	this.getHandleScrollDownFunction = function () {
		return (evt) => {
			this.handleScrollDown(evt);
		};
	};

	this.getHandleScrollUpFunction = function () {
		return (evt) => {
			this.handleScrollUp(evt);
		};
	};
