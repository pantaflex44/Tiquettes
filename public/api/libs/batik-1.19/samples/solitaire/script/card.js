...
function Card(doc, value, suite, deck, faceup) {
	...
	this.faceup = faceup;
	if (this.faceup) {
		this.card.setAttributeNS(xlinkns, "xlink:href", deck + "#" + id);
	} else {
		this.card.setAttributeNS(xlinkns, "xlink:href", deck + "#card-back");
	}
...
Card.prototype.flipCard = function (faceup) {
	...
	if (faceup) {
		this.card.setAttributeNS(xlinkns, "xlink:href", this.deck + "#" + this.id);
	} else {
		this.card.setAttributeNS(xlinkns, "xlink:href", this.deck + "#card-back");
	}
	this.faceup = faceup;
};
...
Card.prototype.updateDisplay = function () {
	...
	var register = false;
	if (deltaX < 0) {
		if (deltaX * 1.01 <= this.dx && deltaX * 0.99 >= this.dx) {
			newX = this.destX;
		} else {
			register = true;
		}
	...
	if (deltaY < 0) {
		if (deltaY * 1.01 <= this.dy && deltaY * 0.99 >= this.dy) {
			newY = this.destY;
		} else {
			register = true;
		}
	...
	if (!register) {
		...
		if (!nc) this.pile.g.appendChild(this.elem);
	}
