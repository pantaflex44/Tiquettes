...
Pile.prototype.addCard = function (card) {
	...
	card.setPos(
		this.x + this.pdx * len + this.dx * len,
...
Pile.prototype.nextCard = function (card) {
	...
	for (var i = 0; i < sz; i++) {
		if (this.cards[i] == card) return this.cards[i + 1];
	}
	return null;
};
...
PileClickHandler.prototype.handleEvent = function (evt) {
	if (this.tgt != evt.target) {
		...
		this.clickCnt = 1;
		if (this.pile.click) this.pile.click(this.pile, evt);
		return;
	}
...
PileMouseDownHandler.prototype.handleEvent = function (evt) {
	...
	new CardMoveManager(g, this.pile, cards, evt.clientX, evt.clientY);
};
...
CardMoveManager.prototype.mousemove = function (evt) {
	...
	if (!this.moved && dx * dx + dy * dy < 4) return;
