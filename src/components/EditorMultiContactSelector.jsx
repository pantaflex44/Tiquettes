...
export default function EditorMultiContactSelector({
	...
	function getModuleTextById(id) {
		return (
			[
				...(filteredModulesListBySchemaFuncs.kc ?? []),
				...(filteredModulesListBySchemaFuncs.tl ?? []),
			].find((m) => m.id === id)?.text ?? ""
		);
	}
						...
						return module.id !== currentModuleId ? (
							<li
								...
								style={{
									...
									paddingInline: "0.5rem",
									paddingBlock: "0.5rem",
									backgroundColor: list.includes(module.id)
							...
							>
								<input
									...
									checked={list.includes(module.id)}
									onChange={(e) => {
										let newList = [];
										if (e.target.checked) {
								...
								/>
								<label htmlFor={`checkbox-${module.id}`} style={{}}>
