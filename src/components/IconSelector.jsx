...
function IconSelector({ value = null, onChange = null, onOpenState = null }) {
	...
	function handleSearchInput(e) {
		...
		const f = [...swbIcons].filter(
			(icon) => icon.title.toLowerCase() === v.toLowerCase(),
		);
	...
	useEffect(() => {
		...
		if (opened && search.filename) {
			const result = Array.from(listRef.current.children).filter(
				(child) => child.dataset.value === search.filename,
			);
	...
	useEffect(() => {
		...
		const f = [...swbIcons].filter((icon) =>
			icon.title.toLowerCase().includes(search.title.toLowerCase()),
		);
	...
	useEffect(() => {
		if (value) {
			...
			if (f.length > 0 && (!selected || f[0].filename !== selected.filename))
				setSelected(f[0]);
		}
	}, [value]);
