	...
	return (
		...
		<li
			...
			style={{
				...
				columnGap: "1em",
				backgroundColor:
					value?.key !== selected?.key && hoveredItem === value?.key
						? "var(--secondary-color)"
		...
		>
			<div
				style={{
					...
					border: value?.key === "" ? "1px solid lightgray" : "initial",
				}}
			></div>
			{value?.key === "" && (
				<div style={{ alignSelf: "center" }}>Aucune couleur</div>
			)}
			{value?.key === "_new_" && (
				<div style={{ alignSelf: "center" }}>Nouvelle couleur ...</div>
			)}
