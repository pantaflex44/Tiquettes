...
export default function TwofaInput({
	...
	useEffect(() => {
		if (
			...
			fetch(
				`${import.meta.env.VITE_APP_API_URL}auth.php?function=twoFaCheck&email=${encodeURIComponent(email)}&token=${encodeURIComponent(debouncedValue)}`,
				{
					method: "GET",
	...
	const handleSend = () => {
		...
		fetch(
			`${import.meta.env.VITE_APP_API_URL}auth.php?function=twoFaSend&email=${encodeURIComponent(email)}`,
			{
				method: "GET",
				headers: { "Content-Type": "application/json" },
			},
	...
	return (
		<>
			{sent ? (
				<>
					<label htmlFor={name}>
						Vous allez recevoir d'ici peu le jeton de sécurité à repporter dans
					...
					<div
						...
						ref={boxRef}
					>
						{Array.from(Array(twofaCodeLength), (_e, i) => (
							<div
								id={`twofaBox_input_${i}`.trim()}
								ref={refs[i]}
								key={i}
								className={`twofaBox_input ${explodedValue[i] === "" ? "empty" : ""} ${verifying ? "disabled" : ""}`.trim()}
								onKeyUp={(e) => {
									if (verifying) return;
					...
					{resendLink && !verifying && (
						// biome-ignore lint/a11y/useValidAnchor: wanted
						<a
							className="twofaBoxResendLink"
							href="#"
							onClick={() => {
								handleSendLink();
							}}
						>
							↺ Je demande un nouveau jeton de sécurité !
						</a>
					)}
				</>
			) : (
				<>
					<label htmlFor={name}>
						Rendez-vous dans votre boite email pour récupérer le jeton de
						sécurité.
					</label>
					<button
						disabled={loading}
						type="button"
						className="twofaBoxSendButton"
						onClick={() => handleSend()}
					>
