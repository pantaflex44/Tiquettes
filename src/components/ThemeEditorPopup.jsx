...
export default function ThemeEditorPopup({
	...
	const invertPosition = (actualId, actualPosition, newPosition) => {
		const found = Object.entries(editedTheme.data).filter(
			(k) => k[1].position === newPosition,
		);
	...
	const up = (el) => {
		...
		if (actualPosition === "middle") {
			...
			if (!invertPosition(el, actualPosition, "middle"))
				invertPosition(el, actualPosition, "top");
		}
	};
	...
	const shown = useMemo(
		...
		[
			editedTheme.data.icon?.shown,
			editedTheme.data.id?.shown,
			editedTheme.data.text?.shown,
		],
	);
	const shownCount = useMemo(
		() =>
			[shown.id ? 1 : 0, shown.icon ? 1 : 0, shown.text ? 1 : 0].reduce(
				(acc, cur) => acc + cur,
				0,
			),
		[shown.icon, shown.id, shown.text],
	);
	const positions = useMemo(() => {
		...
		const idOrder =
			idPosition === "bottom" ? 2 : idPosition === "middle" ? 1 : 0;
		const iconOrder =
			iconPosition === "bottom" ? 2 : iconPosition === "middle" ? 1 : 0;
		const textOrder =
			textPosition === "bottom" ? 2 : textPosition === "middle" ? 1 : 0;
		...
		editedTheme.data.icon?.position,
		editedTheme.data.id?.position,
		editedTheme.data.text?.position,
		shown,
	]);

	const prepareTheme = (t) =>
		Object.keys(defaultTheme).reduce((accumulator, key) => {
		...
		<Popup
			...
			additionalButtons={[
								...
								if (
									...
									fileReader.onload = (e) => {
										try {
											...
											if (!et.data?.id || !et.data?.icon || !et.data?.text) {
												throw new Error("Malformated theme - Bad structure");
											}

											if (
												!("shown" in et.data.id) ||
												!("shown" in et.data.icon) ||
												!("shown" in et.data.text)
											) {
												throw new Error(
													"Malformated theme - Missing properties",
												);
											}
				...
				},
				{
					...
					callback: () => {
						if (
							confirm("Êtes-vous certain de vouloir annuler vos modifications?")
						)
		...
		>
			<div className={"tep-grid"}>
				...
				<div className={"tep-top"}>
							...
							<>
								...
								<div className={"tep-settings_row"} style={{ width: "100%" }}>
									...
									<div className={"tep-settings_row-el"}>
										<GroupColorChooser
											value={
												(editedTheme.data.top?.border ?? false) !== true
													? "#eeeeee"
													: (editedTheme.data.top?.borderColor ?? "#000000")
											}
											disabled={
												(editedTheme.data.top?.border ?? false) !== true
											}
											...
											useGroupColorValue={
												editedTheme.data.top?.colorUseGrp === true
											}
											onUseGroupColorChange={(e) =>
												setEditedTheme((old) => ({
													...old,
													data: {
														...old.data,
											...
											onChange={(e) =>
												setEditedTheme((old) => ({
													...
													data: {
														...
														top: {
															...(old.data.top ?? {}),
															borderSize: parseInt(e.target.value, 10),
														},
													},
												}))
											}
								...
								<div className={"tep-settings_row"} style={{ width: "100%" }}>
									<div className={"tep-settings_row-el"}>
										...
										<input
											type={"checkbox"}
											checked={
												(editedTheme.data.bottom?.border ?? false) === true
											}
									...
									<div className={"tep-settings_row-el"}>
										<GroupColorChooser
											value={
												(editedTheme.data.bottom?.border ?? false) !== true
													? "#eeeeee"
													: (editedTheme.data.bottom?.borderColor ?? "#000000")
											}
											disabled={
												(editedTheme.data.bottom?.border ?? false) !== true
											}
											...
											title={"Couleur de la bordure"}
											useGroupColorValue={
												editedTheme.data.bottom?.colorUseGrp === true
											}
											onUseGroupColorChange={(e) =>
												setEditedTheme((old) => ({
													...old,
					...
					<div
						className={"tep-preview"}
						style={{ width: `calc(${sample.width}mm + 1px + 50px)` }}
					>
						<div
							className={"tep-module"}
							style={{
					...
					<div className={"tep-sample"} style={{}}>
						...
						>
							...
							<IconSelector
								value={sample.icon}
								onChange={(selectedIcon, _selected) => {
						...
						>
							...
							<input
								...
								onChange={(e) => {
									...
									if (value >= heightMin)
										setSample((old) => ({ ...old, height: value }));
								}}
				...
				<div className="tep-extras">
					<b>⁛</b> Télécharger de{" "}
					<a
						href="https://www.tiquettes.fr/themes.php"
						target={"_blank"}
						rel={"noopener"}
					>
						nouveaux thèmes
					</a>{" "}
					pour embellir ses étiquettes !
				</div>
			</div>
		</Popup>
	);
