...
export default function Popup({
	...
	return (
		<div
			className={`popup-overflow ${withOverflow ? "" : "transparent"}`.trim()}
		>
			<div
			...
			>
				...
				<div
					...
					style={
						noPadding
							? { ...style, marginInline: noPadding ? 0 : "1.5rem" }
							: style
					}
				>
					{children}
				...
				<div
					...
					style={{ marginTop: noPadding ? 0 : "1rem" }}
				>
					<div className="popup_buttons_box">
						{Array.isArray(additionalButtons) &&
							additionalButtons.map((b, i) => {
								...
								let p = { ...props };
								if (buttonsDisabled) {
									let cls = (p.className ?? "").replaceAll(/disabled/gi, "");
									cls = `${cls} disabled`;
									p = { ...p, className: cls };
								}
