...
function Row({
	...
	return (
		<div
			className={`switchboard_row row_${rowPosition}`}
			id={`row_${rowPosition}`}
		>
			<div className="row_title">
				<LazyImage
					...
					width={20}
					height={20}
					alt="Supprimer cette rangée"
					title="Supprimer cette rangée"
					onClick={() => {
						if (
							confirm(
								`Supprimer la rangée ${rowPosition} et tout ce qu'elle contient?`,
							)
						)
			...
			<div className="row" style={style}>
				{items.map((item, i) => (
					<Module
						...
						onMoveLeft={(item, moduleRef) =>
							onModuleMoveLeft(i, item, moduleRef)
						}
						onMoveRight={(item, moduleRef) =>
							onModuleMoveRight(i, item, moduleRef)
						}
