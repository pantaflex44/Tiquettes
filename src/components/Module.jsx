...
function Module({
	...
	const canPaste = useMemo(
		() => !isDemo && item.free && hasClipboard && pasteAllowed?.(item),
		[hasClipboard, isDemo, item, pasteAllowed],
	);
	const canInter = useMemo(
		() => !isDemo && !item.free && hasClipboard && interAllowed?.(item),
		[hasClipboard, isDemo, item, interAllowed],
	);
	const canEdit = useMemo(
		() => !isDemo && onEdit && !item.free,
		[isDemo, item.free, onEdit],
	);
	const canCopy = useMemo(
		() => !isDemo && onCopy && !item.free,
		[isDemo, item.free, onCopy],
	);
	const canInterCopy = useMemo(
		() => !isDemo && onInterCopy && !item.free,
		[isDemo, item.free, onInterCopy],
	);
	const canTransform = useMemo(
		() =>
			!isDemo &&
			(moveLeftAllowed?.(item) ||
				moveRightAllowed?.(item) ||
				shrinkAllowed?.(item) ||
				growAllowed?.(item)),
		[
			growAllowed,
	...
	const halfModeLeft = useMemo(
		() => (item.half === "none" || item.half === "right" ? "left" : "none"),
		[item.half],
	);
	const halfModeRight = useMemo(
		() => (item.half === "none" || item.half === "left" ? "right" : "none"),
		[item.half],
	);
	const canHalfMode = useMemo(
		() => !item.free && item.span > 1,
		[item.free, item.span],
	);
	const isHalfLeftMode = useMemo(
		() => item.half === "none" || item.half === "right",
		[item.half],
	);
	const isHalfRightMode = useMemo(
	...
	useEffect(() => {
		...
		const applyTheme = (Content, data) => {
			setThemedModule(() => {
				...
				return Content ? <Content item={item} data={data} /> : null;
			});
		};
		...
		const themeImport = (data) => {
			import(`./ThemeEngine.jsx`)
				...
				.catch(() => {
					console.log(
						`Theme '${currentTheme.name}' error. Default theme will be used.`,
					);
					import(`./ThemeEngine.jsx`)
						.then((selectedTheme) =>
							applyTheme(selectedTheme.default, defaultThemeObj.data),
						)
						.catch((err2) => {
		...
		item && (
			...
			>
				{canPaste ? (
					...
					<div
						...
						style={{
							width: isDemo
								? "calc(100% + 1px)"
								: `calc(100% - (${item.half === "none" ? "0px" : `calc(${style["--sw"]} / 2)`}))`,
							minWidth: isDemo
								? "calc(100% + 1px)"
								: `calc(100% - (${item.half === "none" ? "0px" : `calc(${style["--sw"]} / 2)`}))`,
							maxWidth: isDemo
							...
							"--topSeparatorStyle":
								currentTheme?.data?.top?.border === true
									? (currentTheme?.data?.top?.borderStyle ?? "solid")
									: "initial",
							"--topSeparatorSize":
								currentTheme?.data?.top?.border === true
									? `${currentTheme?.data?.top?.borderSize ?? 1}px`
									: "initial",
							"--topSeparatorColor":
								currentTheme?.data?.top?.border === true
									? currentTheme?.data?.top?.colorUseGrp === true &&
										item.grp &&
										item.grp !== ""
										? item.grp
										: (currentTheme?.data?.top?.borderColor ?? "#000000")
									: "initial",
							"--bottomSeparatorStyle":
								currentTheme?.data?.bottom?.border === true
									? (currentTheme?.data?.bottom?.borderStyle ?? "solid")
									: "initial",
							"--bottomSeparatorSize":
								currentTheme?.data?.bottom?.border === true
									? `${currentTheme?.data?.bottom?.borderSize ?? 1}px`
									: "initial",
							"--bottomSeparatorColor":
								currentTheme?.data?.bottom?.border === true
									? currentTheme?.data?.bottom?.colorUseGrp === true &&
										item.grp &&
										item.grp !== ""
										? item.grp
				...
				{!hasClipboard && !isDemo && canTransform && (
					<div className="module_top">
						<div className="top_row">
							<div
								className="tool"
								title="Demi module sur la gauche"
						...
						<div className="top_row">
							<div
								className="tool"
								title="Demi module sur la droite"
						...
						{item.span > 1 && (
							<div className="top_row" style={{ flex: 1, marginTop: "4px" }}>
								<div
									className="tool size"
									style={{ marginLeft: "auto", marginRight: "10px" }}
									title={`Largeur: ${item.span} modules`}
								>
									<br />⤚ {item.span} ⤙
								</div>
							</div>
				...
				{!canPaste && !canInter && !isDemo && !isFree && (
					<div className="module_bottom">
						...
						<div className="bottom_row" style={{ marginTop: "2px" }}>
							{canInterCopy && (
								<div
									className="tool inter"
									title="Echanger ce module avec un autre"
				...
				{hasClipboard && !isDemo && clipboard?.id === item.id && (
					<div
						className="module_bottom paste force_visible"
						title="Cliquer ici pour annuler"
					>
						<div
							className="tool paste cancel"
							onClick={() => cancelPaste()}
							style={{
								width: "100%",
								height: "100%",
				...
				{canInter && !isDemo && clipboard?.id !== item.id && (
					<div
						className="module_bottom paste"
						title="Cliquer sur le module pour procéder à l'échange"
					>
						<div
							className="tool inter"
							style={{
