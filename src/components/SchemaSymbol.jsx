...
export default function SchemaSymbol({
    ...
    const func = useMemo(() => {
        ...
        const icon = `${import.meta.env.BASE_URL}` + (isContact && (schemaFunctions['kc'].hasNONCChoice === true || schemaFunctions['tl'].hasNONCChoice === true) ? `schema_${func}` + (module.kcType === 'NC' ? '_nc' : '') + '.svg' : `schema_${func}.svg`)
