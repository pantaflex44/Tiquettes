...
export default function NewProjectPopup({
	...
	const Summary = () => {
		return (
			<>
				<span
					className={`wizard-page-left-title ${step === 1 ? "current" : ""}`.trim()}
				>
					Bienvenue !
				</span>
				<span
					className={`wizard-page-left-title ${step === 2 ? "current" : ""}`.trim()}
				>
					Mon enveloppe
				</span>
				<span
					className={`wizard-page-left-title ${step === 3 ? "current" : ""}`.trim()}
				>
					Données du projet
				</span>
				<span
					className={`wizard-page-left-title ${step === 4 ? "current" : ""}`.trim()}
				>
					Résumons
				</span>
			</>
		...
		<>
			<Popup
				...
				onCancel={() => {
					if (
						confirm(
							"Voulez-vous abandonner la création de votre nouveau projet ?",
						)
					) {
						cancel();
					}
			...
			>
				{step === 1 && (
					<div className="wizard-page" data-step={step}>
						...
						<div className="wizard-page-right">
							...
							<p>
								Vous allez configurer un nouveau projet en suivant les quelques
								étapes nécessaires. C'est simple et très rapide.
							</p>
							<p>
								Laissez vous guider, et retrouvons nous à la fin pour entrer
				...
				{step === 2 && (
					<div className="wizard-page" data-step={step}>
						...
						<div className="wizard-page-right">
							<h3>Commençons par définir l'enveloppe du tableau électrique</h3>
							<div className="wizard-page-form">
								<div
									className="wizard-page-form-row"
									onMouseEnter={() => {
								...
								>
									<label htmlFor="rowsCount">
										Mon enveloppe comporte <b>{properties.rowsCount}</b> rangée
										{properties.rowsCount > 1 ? "s" : ""}
									</label>
									<div className="wizard-page-form-row-flex">
										<span className="wizard-page-form-row-indicator-left">
											{rowsMin}
										</span>
										<input
											style={{ flex: 1 }}
											...
											onChange={(e) =>
												setProperties((old) => ({
													...old,
													rowsCount: parseInt(e.target.value, 10),
												}))
											}
										/>
										<span className="wizard-page-form-row-indicator-right">
											{rowsMax}
										</span>
									</div>
								</div>
								<div
									...
									onMouseEnter={() => {
										...
										setTooltipBox(
											"Le nombre de modules par rangée est normalisé. Vous retrouverez souvent les nombres de 13, 18 et 24 modules par rangée.",
										);
									}}
									onMouseLeave={() => {
										setWizardPicture2src(wizardPicture2);
								...
								>
									...
									<div className="wizard-page-form-row-flex">
										<div className="radio_group" style={{ margin: 0 }}>
											{import.meta.env.VITE_ALLOWED_MODULES.split(",").map(
												(count) => {
													const c = parseInt(count.trim(), 10);
													return (
														<div key={c} className="radio">
															<input
																type="radio"
																name={`stepsPerRows_${c}`}
																id={`stepsPerRows_${c}`}
																value={c}
																...
																onChange={(e) =>
																	setProperties((old) => ({
																		...old,
																		stepsPerRows: parseInt(e.target.value, 10),
																	}))
																}
															/>
															<label
																htmlFor={`stepsPerRows_${c}`}
																style={{
																	margin: 0,
																	marginRight: "0.5em",
																	paddingInline: "1em",
								...
								<div
									...
									onMouseEnter={() => {
										...
										setTooltipBox(
											"Certains modules anciens possèdent une largeur de 17.5mm. Aujourd'hui il est courant d'utiliser des modules de 18mm de largeur.",
										);
									}}
									onMouseLeave={() => {
										setWizardPicture2src(wizardPicture2);
								...
								>
									<label htmlFor="stepSize">
										Et chaque module a une largeur de
									</label>
									<div className="wizard-page-form-row-flex">
										<div className="radio_group" style={{ margin: 0 }}>
											{[17.5, 18].map((s) => {
												return (
													<div key={s} className="radio">
														<input
															type="radio"
															name={`stepSize_${s}`}
															id={`stepSize_${s}`}
								...
								<div
									...
									onMouseEnter={() => {
										...
										setTooltipBox(
											"La hauteur des étiquettes dépend de l'espace disponible proposé par l'enveloppe choisie. Chaque marque / modèle possède ses propres dimensions. Pour garantir une insertion correcte des étiquettes imprimées, il est conseillé de retirer 1mm de la hauteur mesurée.",
										);
									}}
									onMouseLeave={() => {
										setWizardPicture2src(wizardPicture2);
								...
								>
									<label htmlFor="height">
										Les portes étiquettes font <b>{properties.height}mm</b> de
										hauteur
									</label>
									<div className="wizard-page-form-row-flex">
										<span className="wizard-page-form-row-indicator-left">
						...
						<div className="wizard-page-right">
							...
							<div className="wizard-page-form">
								<div className="wizard-page-form-row">
									...
									<div className="wizard-page-form-row-flex">
										<input
											style={{ flex: 1, height: "30px", fontSize: "90%" }}
											type="text"
											name="name"
											id="name"
									...
									<span className="wizard-form-tooltip">
										Définissez un nom simple et clair pour mieux reconnaitre
										votre projet.
									</span>
								</div>
								<div className="wizard-page-form-row">
									...
									<div style={{ fontSize: "95%" }}>
										Vous avez la possibilité de renseigner vos informations en
										tant qu'installateur mais aussi celles de votre client.
									</div>
									<div className="wizard-page-form-row-flex">
										<button
											type="button"
									...
									<div style={{ fontSize: "95%" }}>
										Vous pourrez modifier ultérieurement ces données dans le
										menu Imprimer / Page de garde / Paramètres.
									</div>
								</div>
							</div>
						</div>
				...
				{step === 4 && (
					<div className="wizard-page" data-step={step}>
						...
						<div className="wizard-page-right">
							<h3>Résumons votre projet</h3>
							<p>
								Mon projet repose sur une enveloppe de{" "}
								<b>
									{properties.rowsCount} rangée
									{properties.rowsCount > 1 ? "s" : ""}
								</b>{" "}
								comportant <b>{properties.stepsPerRows} modules</b> chacune.
							</p>
							<p>
								Chaque module fait <b>{properties.stepSize}mm de largeur</b>.
							</p>
							<p>
								Vous avez renseigné une hauteur de <b>{properties.height}mm</b>{" "}
								pour l'emplacement dédié aux étiquettes.
							</p>
							<p>
								Pour mieux l'identifier, votre projet porte le nom{" "}
								<b>{properties.name}</b>.
							</p>
							{(properties.infos.from.name ||
								...
								<p>
									Vous avez défini les informations relatives à l'installateur.
								</p>
							)}
							{(properties.infos.to.name ||
								...
								<p>Vous avez défini les informations relatives au client.</p>
							)}
							<p style={{ marginTop: "5rem" }}>
								Si ces informations sont exactes, cliquez sur le bouton{" "}
								<b>Valider</b> pour créer votre projet.
							</p>
						</div>
					</div>
				)}
