...
export default function Editor({
	...
	const prevModuleTitle = useMemo(
		() =>
			(
				(prevModule?.id ?? "-") +
				" " +
				(prevModule && schemaFunctions[prevModule.func]
					? `(${schemaFunctions[prevModule.func].name})`
					: "")
			).trim(),
		[prevModule],
	);
	...
	const onUpdateModuleEditor = (data) => {
		setEd((old) => ({
			...old,
			currentModule: { ...old.currentModule, ...data },
		}));
	};
	...
	const parentModuleIsTri = useMemo(
		() =>
			parentModule &&
			schemaFunctions[parentModule.func]?.hasPole &&
			...
			<Popup
				title={
					<div className="popup_title_content">
						...
						/>
						<span className="popup_title_content_id">
							{ed.currentModule.id ?? ""}
						</span>
						{ed.currentModule.text ? (
							<span className="popup_title_content_desc">
								/ {ed.currentModule.text ?? ""}
							</span>
						) : (
							""
			...
			>
				<div className="popup_rows" style={{ minHeight: "710px" }}>
					<div className={"editor_tabpages"}>
						...
						<input
							type="checkbox"
							id="schema_editor_tab"
							checked={editorTab === "schema"}
					...
					{editorTab === "main" && (
						<>
							<div
								...
								style={{ "--left_column_size": "120px" }}
							>
								<label htmlFor={`editor_id_${ed.currentModule.id.trim()}`}>
									Identifiant
								</label>
								<div className="popup_row-flex">
									...
									<button
										...
										title={
											ed.currentModule.noAutoId === true
												...
												: "Permettre la re-numérotation automatique de cet identifiant"
										}
										onClick={() =>
											onUpdateModuleEditor({
												noAutoId: !ed.currentModule.noAutoId,
											})
										}
										style={{
											backgroundColor:
									...
									>
										<img
											...
											alt="Re-numérotation automatique."
										/>
									</button>
								</div>
							</div>
							...
							<div
								...
								style={{ "--left_column_size": "120px" }}
							>
								<label htmlFor={`editor_text_${ed.currentModule.id.trim()}`}>
									Libellé
								</label>
								<textarea
									name="editor_text"
									...
									onChange={(e) =>
										onUpdateModuleEditor({ text: e.target.value })
									}
									rows={3}
							...
							<div
								...
								style={{
									...
									gridTemplateColumns:
										"var(--left_column_size) calc(100% - 100px)",
									maxWidth: "100%",
								}}
							>
								<label>Couleur</label>
								...
								>
									...
									>
										Regrouppez vos circuits en utilisant des couleurs
										personnalisées.
									</div>
								</div>
							</div>
								...
								>
									<IconSelector
										...
										onChange={(selectedIcon, selected) => {
											if (
												!ed.currentModule.icon ||
												(selectedIcon && ed.currentModule.icon !== selectedIcon)
											) {
												onUpdateModuleEditor({
													icon: selectedIcon,
												...
												if (selected?.wire && ed.currentModule.wire === "") {
													...
													if (ed.currentModule.current !== "") {
														const c = parseInt(
															(ed.currentModule.current ?? "").replace(
																/\D/g,
																"",
															),
															10,
													...
													onUpdateModuleEditor({
														wire: w > 0 ? `${w}` : selected?.wire,
													});
												}

												if (selected?.modtype && !isCustomFunction)
													onUpdateModuleEditor({
														modtype: selected?.modtype,
													});
											}
							...
							>
								<label
									htmlFor={`editor_modtype_${(ed.currentModule?.modtype ?? "").trim()}`}
								>
									Type
								</label>
								<input
							...
							>
								...
								<textarea
									...
									onChange={(e) =>
										onUpdateModuleEditor({ desc: e.target.value })
									}
									rows={2}
					...
					{editorTab === "schema" && (
						<>
							<div
								...
								style={{ "--left_column_size": "120px" }}
							>
								<label htmlFor={`editor_func_${ed.currentModule.id.trim()}`}>
									Fonction
								</label>
								<div
									className="popup_row-grid"
									style={{ gridTemplateColumns: "1fr 67px" }}
								>
									<EditorFunctionSelector
										...
										onChange={(value) => {
											...
											if (
												schemaFunctions[ed.currentModule.func]
													?.hasShareWithChilds !== true
											) {
										...
										title={
											schemaFunctions[ed.currentModule.func]?.description ??
									...
									<EditorParallelSelector
										...
										onChange={(value) =>
											onUpdateModuleEditor({ onlyChilds: value })
										}
									/>
								</div>
							</div>
							...
							{ed.currentModule.func && (
								<>
									...
									>
										<label
											htmlFor={`editor_schparent_${ed.currentModule.id.trim()}`}
										>
											Parent
										</label>
										<div
							...
							{ed.currentModule.func &&
								schemaFunctions[ed.currentModule.func]?.supportContacts ===
									true && (
									<div
										className="popup_row"
										style={{
									...
									>
										<label
											htmlFor={`editor_contacts_${ed.currentModule.id.trim()}`}
										>
											Asservi par
										</label>
										<div
							...
							{schemaFunctions[ed.currentModule.func]?.hasType && (
								<div
									...
									style={{ "--left_column_size": "120px" }}
								>
									<label htmlFor={`editor_type_${ed.currentModule.id.trim()}`}>
										Type
									</label>
									<EditorTypeSelector
							...
							{schemaFunctions[ed.currentModule.func]?.hasCrb && (
								<div
									...
									style={{ "--left_column_size": "120px" }}
								>
									<label htmlFor={`editor_crb_${ed.currentModule.id.trim()}`}>
										Courbe
									</label>
									<EditorCrbSelector
							...
							{schemaFunctions[ed.currentModule.func]?.hasType && (
								...
								>
									<label
										htmlFor={`editor_sensibility_${ed.currentModule.id.trim()}`}
									>
										Sensibilité
									</label>
									<EditorSensibilitySelector
										id={`editor_sensibility_${ed.currentModule.id.trim()}`}
										value={ed.currentModule.sensibility}
										onChange={(value) =>
											onUpdateModuleEditor({ sensibility: value })
										}
									/>
								</div>
							)}
							...
							{schemaFunctions[ed.currentModule.func]?.hasNONCChoice && (
								...
								>
									<label
										htmlFor={`editor_current_${ed.currentModule.id.trim()}`}
									>
										Type
									</label>
									<EditorContactTypeSelector
										...
										onChange={(value) =>
											onUpdateModuleEditor({ kcType: value })
										}
									/>
								</div>
							)}
							...
							{schemaFunctions[ed.currentModule.func]?.hasCurrent && (
								...
								>
									...
									<EditorCurrentSelector
										...
										onChange={(value) =>
											onUpdateModuleEditor({ current: value })
										}
									/>
								</div>
							)}
							...
							{schemaFunctions[ed.currentModule.func]?.hasWire && (
								<div
									...
									style={{ "--left_column_size": "120px" }}
								>
									<label htmlFor={`editor_wire_${ed.currentModule.id.trim()}`}>
										Section
									</label>
									<EditorWireSelector
										...
										current={parseInt(
											(ed.currentModule.current ?? "").replace(/\D/g, ""),
											10,
										)}
							...
							{schemaFunctions[ed.currentModule.func]?.hasPole && (
								<div
									...
									style={{ "--left_column_size": "120px" }}
								>
									<label htmlFor={`editor_pole_${ed.currentModule.id.trim()}`}>
										Pôles
									</label>
									<EditorPoleSelector
