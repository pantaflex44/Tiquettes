...
function CustomTheme({ item, data, style }) {
	...
	const iconColor = useMemo(() => {
		try {
			const rgb = hexToRgb(
				colors.fg.icon ? item.grp : (data?.icon?.color ?? "#000000"),
			);
			if (rgb.length === 3) {
	...
	const idLineCount = useMemo(() => {
		try {
			const lineCount = parseInt(data?.id?.lineCount ?? "1", 10);
	...
	const textLineCount = useMemo(() => {
		try {
			const lineCount = parseInt(data?.text?.lineCount ?? "1", 10);
			if (lineCount < 1) return 1;
			return lineCount;
		} catch {
			return 1;
		...
		() => ({
			...
			icon: {
				...
				backgroundColor: colors.bg.icon
					? item.grp
					: (data?.icon?.backgroundColor ?? "transparent"),
			},
			iconImg: {
			...
			text: {
				...
				backgroundColor: colors.bg.text
					? item.grp
					: (data?.text?.backgroundColor ?? "transparent"),
				display: "flex",
				flexDirection: "row",
				flexWrap: "nowrap",
				justifyContent: "center",
			...
			textContent: {
				...
				width: "100%",
				transform:
					(data?.text?.displayMode ?? "H") === "V" ? "rotate(-90deg)" : "none",
			},
		}),
		...
		<>
			{shown.id && (
				<div
					style={styles.id}
					data-order={
						positions.id.order === 0
							? "top"
							: positions.id.order === shownCount - 1
								? "bottom"
								: "middle"
					}
				>
			...
			{shown.icon && (
				<div
					style={styles.icon}
					data-order={
						positions.icon.order === 0
							? "top"
							: positions.icon.order === shownCount - 1
								? "bottom"
								: "middle"
					}
				>
			...
			{shown.text && (
				<div
					style={styles.text}
					data-order={
						positions.text.order === 0
							? "top"
							: positions.text.order === shownCount - 1
								? "bottom"
								: "middle"
					}
				>
