	...
	return !positions[propName].shown ? (
		...
		<>
			<div className={"tep-settings_row"}>
				<label htmlFor={"tep-icon-fullheight"}>
			...
			<div className={"tep-settings_row"}>
				...
				<select
					name="sample_modtype"
					id={"sample_modtype"}
					value={editedTheme.data[propName]?.type ?? "icon"}
					onChange={(e) =>
						setEditedTheme((old) => ({
							...old,
							data: {
			...
			<div className={"tep-settings_row"}>
				...
				<input
					type={"range"}
					value={editedTheme.data[propName]?.sizePercent ?? 50}
					min={0}
					max={100}
				...
				>{`${editedTheme.data[propName]?.sizePercent ?? 50}%`}</span>
			</div>
